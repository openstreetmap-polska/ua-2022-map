...
function renderPopupRouteLink(lonlat, properties, text, href) {
...
function renderBasicPopup(lonlat, properties) {
    ...
    var popupHTML = `
        ...
        ${renderPopupRouteLink(lonlat, properties, `OpenStreetMap {{ strings.navigation[lang] }}`, `https://www.openstreetmap.org/directions?from=&to=${lonlat[1]}%2C${lonlat[0]}#map=14/${lonlat[1]}/${lonlat[0]}`)}
        ${renderPopupRouteLink(lonlat, properties, `Google Maps {{ strings.navigation[lang] }}`, `https://www.google.com/maps/dir/?api=1&destination=${lonlat[1]}%2C${lonlat[0]}`)}
        ${renderPopupRouteLink(lonlat, properties, 'GeoURI', `geo:${lonlat[1]},${lonlat[0]}`)}
